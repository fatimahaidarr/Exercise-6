...
int threshold = 33;
float contourDetail = 0.1; // smaller values mean more detail
boolean armOpenCvUpdate = false;
float scaler = 2;
float minContourArea = 5;
float maxContourArea = 12000;
  ...
  if (armOpenCvUpdate) {
    ...
    for (Contour contour : contours) {   
      ...
      if (area > minContourArea && area < maxContourArea) {
        ...
        childRect.setStroke(color(255, 0, 100));
